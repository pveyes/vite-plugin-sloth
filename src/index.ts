...
async function transformHtml(html: string, options: Options) {
  ...
  const externalTemplates: Template[] = await Promise.all(
    $('link')
      .filter((_, link) => {
        return link.attribs.rel === 'import' && (link.attribs.href.endsWith('html') || link.attribs.href.startsWith('data:text/html'))
      })
      .map((_, link) => {
        ...
        try {
          if (link.attribs.href.startsWith('data:')) {
            const template = Buffer.from(link.attribs.href.replace('data:text/html;base64,', ''), 'base64').toString();
            return {
              content: template,
            }
          }
  ...
  const inlineTemplates = $('template').map((_, template) => {
    return {
  ...
  templates.forEach(template => {
    ...
    customElements.each((_, el) => {
      const target = $(el);
      const content =
        compileContent(target, template.content, {
...
function compileContent(target: Cheerio<any>, template: string, options: CompileOptions) {
    ...
    .filter((_, el) => {
      ...
      Object.keys(elementData).forEach(key => {
        if (variables[key] === undefined && !$el.attr(key)) {
          console.warn(`Template requires data-${key} but it's missing`);
        } else {
